...
from spformer.utils import boundary_detection
...
class B3DDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.logger.info(f'Load dataset: {len(self.filenames)} scans')
    ...
    def transform_train2(self, xyz, rgb, superpoint, semantic_label, instance_label,boundary):
        xyz_middle = self.data_aug(xyz, True, True, True)
        rgb = rgb
        boundary = boundary
        xyz = xyz_middle * self.voxel_cfg.scale
        ...
        superpoint = np.unique(superpoint[valid_idxs], return_inverse=True)[1]
        instance_label = self.get_cropped_inst_label(instance_label, valid_idxs)
        return xyz, xyz_middle, rgb, superpoint, semantic_label, instance_label,boundary
    ...
    def transform_test(self, xyz, rgb, superpoint, semantic_label, instance_label):
        ...
        instance_label = self.get_cropped_inst_label(instance_label, valid_idxs)
        return xyz, xyz_middle, rgb, superpoint, semantic_label, instance_label
    def transform_test2(self, xyz, rgb, superpoint, semantic_label, instance_label,boundary):
        xyz_middle = xyz
        xyz = xyz_middle * self.voxel_cfg.scale
        xyz -= xyz.min(0)
        valid_idxs = np.ones(xyz.shape[0], dtype=bool)
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        data = self.transform_train2(*data) if self.training else self.transform_test2(*data)
        xyz, xyz_middle, rgb, superpoint, semantic_label, instance_label, boundary = data
        k = 20
        xyz_32 = xyz.astype(np.float32)

        knn, _ = pgeof.knn_search(xyz_32, xyz_32, 20)
        num_points = len(xyz)
        nn_ptr = np.arange(num_points + 1) * k
        nn = knn.flatten()
        nn_ptr = nn_ptr.astype("uint32")
        nn = nn.astype("uint32")
        features = pgeof.compute_features(xyz_32, nn, nn_ptr)
        f = torch.from_numpy(features.astype('float32'))
        device = torch.device('cpu')

        features_list = []
        if self.features:
            if 'linearity' in self.features:
                features_list.append(f[:, 0].to(device).view(-1, 1))
            if 'planarity' in self.features:
                features_list.append(f[:, 1].to(device).view(-1, 1))
            if 'scattering' in self.features:
                features_list.append(f[:, 2].to(device).view(-1, 1))

            if 'verticality' in self.features:
                features_list.append(f[:, 3].to(device).view(-1, 1))

            if 'curvature' in self.features:
                features_list.append(f[:, 10].to(device).view(-1, 1))

            if 'log_length' in self.features:
                features_list.append(torch.log(f[:, 7] + 1).to(device).view(-1, 1))

            if 'log_surface' in self.features:
                features_list.append(torch.log(f[:, 8] + 1).to(device).view(-1, 1))

            if 'log_volume' in self.features:
                features_list.append(torch.log(f[:, 9] + 1).to(device).view(-1, 1))

            if 'boundary' in self.features:
                features_list.append(torch.from_numpy(boundary).to(device).view(-1, 1))
            if features_list:  
                features_nd = torch.cat(features_list, dim=1)
        else:
            features_nd = torch.from_numpy(rgb).float()
        ...
        instance_label = np.where(instance_label < 1, -100, instance_label - 1)
