...
class Voxelization_Idx(Function):
    ...
    def forward(ctx, coords, batchsize, mode=4):
        '''
        :param ctx:
        # ctx：上下文对象，通常用于保存计算过程中的信息。
        :param coords:  long (N, dimension + 1) or (N, dimension) dimension = 3
        # 一个长整型张量，形状为(N, dimension + 1)或(N, dimension)，其中dimension为3。表示N个坐标点的三维坐标。
        :param batchsize
        # 批量大小，表示坐标点属于多少个不同的样本。
        :param mode: int 4=mean
        # 一个整数，表示体素化的方式，默认为4，即使用平均值进行体素化。
        # 返回值包括三个部分：
        :param dimension: int
        :return: output_coords:  long (M, dimension + 1) (M <= N)
        一个长整型张量，形状为(M, dimension + 1)，其中M小于或等于N。表示经过体素化后的坐标点
        :return: output_map: int M * (maxActive + 1)
        一个整型张量，形状为M。表示体素化后的坐标点到输出的映射。
        :return: input_map: int N
        一个整型张量，形状为N。表示原始坐标点到体素化的映射。
        '''
