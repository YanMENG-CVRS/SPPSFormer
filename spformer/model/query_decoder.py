...
from KAN.fftKAN import NaiveFourierKANLayer as KANlayer
# test kansformer
...
class QueryDecoder(nn.Module):
    ...
    def __init__(
        ...
        if KAN:
            ...
            self.out_crosspoint = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, 2))

            self.x_mask = nn.Sequential(nn.Linear(in_channel, d_model), nn.ReLU(),KANlayer(d_model,d_model,gridsize=KAN_gridsize))
    ...
    def prediction_head(self, query, mask_feats, batch_offsets):
        ...
        pred_crosspoint = self.out_crosspoint(query)
        pred_masks, attn_masks = self.get_mask(query, mask_feats, batch_offsets)
        return pred_labels, pred_scores, pred_masks, attn_masks,pred_crosspoint
    ...
    def forward_iter_pred(self, x, batch_offsets):
        ...
        if getattr(self, 'pe', None):
            ...
            pe = None
        pred_labels, pred_scores, pred_masks, attn_masks, pred_crosspoint = self.prediction_head(query, mask_feats, batch_offsets)
        prediction_labels.append(pred_labels)
        prediction_scores.append(pred_scores)
        prediction_masks.append(pred_masks)
        prediction_crosspoints.append(pred_crosspoint)
